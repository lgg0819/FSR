...
#		event_num = 3000
		...
		for i in range(event_num):
			...
			for j in range(t.nElectron):
				temp_mass.append(t.Electron_mass.__getitem__(j))
				temp_pt.append(t.Electron_pt.__getitem__(j))
				temp_eta.append(t.Electron_eta.__getitem__(j))
				temp_phi.append(t.Electron_phi.__getitem__(j))
				temp_dxy.append(t.Electron_dxy.__getitem__(j))
				temp_dz.append(t.Electron_dz.__getitem__(j))
				temp_charge.append(t.Electron_charge.__getitem__(j))
				if (t.Electron_mvaTTH.__getitem__(j) == True): tight += 1
				temp_medid.append(t.Electron_mvaTTH.__getitem__(j))
			...
			for k in range(t.nElectron):
				...
				if (temp_mass[k] == 0): continue
				if (temp_pt[k] > 30 and abs(temp_eta[k]) < 2.4 and abs(temp_dxy[k]) < 0.05 and abs(temp_dz[k]) < 0.1 and temp_medid[k] == True):
...
#			higgs_select = min(mass_cand)
			...
			for k in range(t.nJet-2):
				if (temp_pt[k] > w_mass): continue
				if (temp_pt[k] > 0 and abs(temp_eta[k]) < 2.4 and temp_jetid[k] == 7):
					for l in range(k+1,t.nJet-2):
...
#		Bounds(-2.4,2.4)
		...
		opt = minimize(defineTopness, ini, bounds=bnd, method='SLSQP') #options={'disp':True})

		nu_p4 = TLorentzVector()
		nu_p4.SetPtEtaPhiM(pt, opt.x[0], phi, 0)

		w1_p4 = lep1_p4 + lep2_p4
		w2_p4 = jet3_p4 + jet4_p4
	...
	c1 = TCanvas("c1","W_onshell Invariant Mass Distribution", 900,600)
	c2 = TCanvas("c2","W_offshell Invariant Mass Distribution", 900,600)

	hist_on = TH1D("h1","W_onshell Invariant Mass Distribution", 40, 30,130)
	hist_off = TH1D("h1","W_offshell Invariant Mass Distribution", 40, 0,300)

	for i in range(len(w_on)):
		hist_on.Fill(w_on[i])

	for i in range(len(w_off)):
		hist_off.Fill(w_off[i])

	c1.cd()
	hist_on.Draw()
	c1.SaveAs("w_on_mass.png")

	c2.cd()
	hist_off.Draw()
	c2.SaveAs("w_off_mass.png")



#		if opt.success: topness.append(opt.fun)
#
#		# Save CSV file
...
#				 'nu_pt','nu_eta','nu_phi','topness'])
#
#        df.to_csv("topness_hh_e.csv", header=False, index=False)
#
