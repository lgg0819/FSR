	...
	for fname in flist:
		...
		for i in range(event_num):
			...
			for k in range(len(temp_mass)-1,0,-1):
				...
				if (temp_pt[k] > 25 and abs(temp_eta[k]) < 2.4 and temp_jetid[k] >= 7):
					for l in range(k-1,0,-1):
						if (flag_jet2 == True): break
						if (temp_pt[l] > 25 and abs(temp_eta[l]) < 2.4 and temp_jetid[l] >= 7):
							...
							jet3_p4.SetPtEtaPhiM(lq_pt,lq_eta,lq_phi,lq_mass)
							jet4_p4.SetPtEtaPhiM(sq_pt,sq_eta,sq_phi,sq_mass)
							diq_p4 = jet3_p4 + jet4_p4

							if (diq_p4.M() >= w_mass): continue
							if (diq_p4.M() < w_mass): flag_jet2 = True


			# =================================================================
			if (flag_lep == True and flag_jet1 == True and flag_jet2 == True):
...
#		Bounds(-2.4,2.4)
		...
		data.append([jet1_p4.M(),jet1_p4.Pt(),jet1_p4.Eta(),jet1_p4.Phi(),
			...
			pt,opt.x,phi,defineHiggsness(),opt.fun,1])
