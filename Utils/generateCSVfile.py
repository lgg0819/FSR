...
def main():
	...
	dr_ll,dr_jj,met_pt,met_phi,genWeight = [],[],[],[],[]
	for fname in flist:
			...
			for k in range(t.nMuon):
				...
				if (temp_pt[k] > 25 and abs(temp_eta[k]) < 2.4 and abs(temp_dxy[k]) < 0.05 and abs(temp_dz[k]) < 0.1 and temp_iso[k] < 0.4 and temp_sip3d[k] < 8 and temp_mva[k] > 0.5 and temp_medid[k] == True):
					for l in range(k+1,t.nMuon):
						if (flag_lep == True): break
						if (temp_pt[l] > 15 and abs(temp_eta[l]) < 2.4 and abs(temp_dxy[l]) < 0.05 and abs(temp_dz[l]) < 0.1 and temp_iso[l] < 0.4 and temp_sip3d[l] < 8 and temp_mva[l] > 0.5  and temp_medid[l] == True):
...
#							mass.append(abs(jet_p4.M()-higgs_mass))
#							if (btag_good == False): mass.pop(-1)
#			if (mass == []): continue
#			higgs_select = min(mass)
		...
		for i in range(len(jet1_mass)):
			...
			H1_mass.append(h1_p4.M())
			H1_pt.append(h1_p4.Pt())
			H1_eta.append(h1_p4.Eta())
			H1_phi.append(h1_p4.Phi())

			H2_mass.append(h2_p4.M())
			H2_pt.append(h2_p4.Pt())
			H2_eta.append(h2_p4.Eta())
			H2_phi.append(h2_p4.Phi())

			HH_mass.append(hh_p4.M())
			HH_pt.append(hh_p4.Pt())
			HH_eta.append(hh_p4.Eta())
			HH_phi.append(hh_p4.Phi())
		...
		for i in range(len(jet1_mass)):
			data.append([jet1_mass[i],jet1_pt[i],jet1_eta[i],jet1_phi[i],jet1_btag[i],
				...
				HH_mass[i],HH_pt[i],HH_eta[i],HH_phi[i],genWeight[i]])
	...
	df.to_csv("signal.csv", header=True, index=False)

	print("Have processed %i files..."%(ii))
	print("Have processed total %i events..."%(ee))
	print("Event number final processed is %i..."%(hh))
...
if __name__ == "__main__":
	...
	jname = "/cms/ldap_home/chdlalsnr/HH/CMSSW_11_0_0/src/HHAnalysis/HH/python/5_Make_Control_Plots/jec/jetl1corr_2016.txt"
	j = open(jname,"read")
