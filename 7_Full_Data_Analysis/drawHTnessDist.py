...
def main():
	...
	hh = open('htness_sig.csv','r')
	hhline = csv.reader(hh)

	tt = open('htness_bkg.csv','r')
	...
	c = TCanvas("c","Histogram in (H,T) Space Background Events_DL", 900,600)

	h2 = TH2D("h2","Histogram in (H,T) Space Background Events_DL", 50,0,10, 50,0,5)
