...
def main(dataset): 
        ...
        with open("dataset_2018_data/%s.dat"%(dataset),'rb') as js:
	...
	H1_mass_cand,H1_pt_cand,H1_eta_cand,H1_phi_cand = [],[],[],[]
        ...
        for fname in flist:
			...
			print("exception act due to AttributeError...")
			continue
		...
		for i in range(event_num):
			...
			for k in range(len(temp_mass)):
				...
				if (temp_pt[k] > 25 and abs(temp_eta[k]) < 2.4 and temp_btag[k] > 0.2770 and temp_jetid[k] >= 7):
                                        for l in range(k+1,len(temp_mass)):
						if (flag_jet == True): break
                                                if (temp_pt[l] > 25 and abs(temp_eta[l]) < 2.4 and temp_btag[l] > 0.2770 and temp_jetid[l] >= 7):
...
def readRunLumi(runs,start,end):
        ...
        fname = "/cms/ldap_home/chdlalsnr/HH/CMSSW_11_0_0/src/HHAnalysis/HH/python/5_Make_Control_Plots/json/runlumi_2018.txt"
        f = open(fname,"read")
